...
static void performJoin(Database &db, Table &table1, Table &table2,
                        ...
                        JoinType type)
{
    ...
    // ── hash-based matching (branch point by join type) ───────────────────────
    ...
    {
        // Index table2; drive over table1 so every table1 row is visited.
    ...
    {
        // Index table1; drive over table2 so every table2 row is visited.
    ...
    // ── display + cleanup ─────────────────────────────────────────────────────
    const char *label = (type == JoinType::INNER) ? "Inner"
                      : (type == JoinType::LEFT)  ? "Left"
                      : (type == JoinType::RIGHT) ? "Right"
                                                  : "Full";
    cout << label << " Join Result:" << endl;
    joinedTable.displayTable(projectedColumns);
    drop(db, "temp_joined_table");
}

void innerJoin(Database &db, Table &table1, Table &table2,
               const vector<string> &joinColumns, const vector<string> &projectedColumns)
{
    performJoin(db, table1, table2, joinColumns, projectedColumns, JoinType::INNER);
}

void leftJoin(Database &db, Table &table1, Table &table2,
              const vector<string> &joinColumns, const vector<string> &projectedColumns)
{
    performJoin(db, table1, table2, joinColumns, projectedColumns, JoinType::LEFT);
}

void rightJoin(Database &db, Table &table1, Table &table2,
               const vector<string> &joinColumns, const vector<string> &projectedColumns)
{
    performJoin(db, table1, table2, joinColumns, projectedColumns, JoinType::RIGHT);
}

void fullJoin(Database &db, Table &table1, Table &table2,
              const vector<string> &joinColumns, const vector<string> &projectedColumns)
{
    performJoin(db, table1, table2, joinColumns, projectedColumns, JoinType::FULL);
}
